<template>
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="150px" align="center" label="等级名称">
        <template slot-scope="scope">
          <!-- <span>{{scope.row.timestamp | parseTime('{y}-{m}-{d} {h}:{i}')}}</span> -->
            <span>{{scope.row.rankName}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="150px" label="开始经验">
        <template slot-scope="scope">
          <!-- <span class="link-type" @click="handleUpdate(scope.row)">{{scope.row.title}}</span>
          <el-tag>{{scope.row.type}}</el-tag> -->
          <span>{{scope.row.startScore}}</span>

        </template>
      </el-table-column>
      <el-table-column min-width="150px" label="结束经验">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="150px" label="描述">
        <template slot-scope="scope">
          <span>{{scope.row.rankdesc}}</span>
        </template>
      </el-table-column>
          <el-table-column min-width="150px" label="手续费">
        <template slot-scope="scope">
          <!-- <span class="link-type" @click="handleUpdate(scope.row)">{{scope.row.title}}</span>
          <el-tag>{{scope.row.type}}</el-tag> -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
        <el-form-item label="等级名称" prop="skills_name">
          <el-input v-model="temp.rankName"></el-input>
        </el-form-item>

        <el-form-item label="开始经验" prop="describe">
          <el-input v-model="temp.startScore"></el-input>
        </el-form-item>
      <el-form-item label="结束经验" prop="icon">
          <el-input v-model="temp.endScore"></el-input>
        </el-form-item>
          <el-form-item label="描述" prop="icon">
          <el-input v-model="temp.rankdesc"></el-input>
        </el-form-item>
            <el-form-item label="手续费率" prop="icon">
          <el-input v-model="temp.poundage"></el-input>
        </el-form-item>
      </el-form>
...
<script>
import { UserList,statusUser,update,create,Userlevel,Delete } from '@/api/user'
...
export default {
  ...
  methods: {
    getList() {
      ...
      Userlevel(this.listQuery).then(response => {

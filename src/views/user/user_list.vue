    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="100px" align="center" label="用户名">
        <template slot-scope="scope">
          ...
          <el-table-column  min-width="100px" align="center" label="用户余额">
        <template slot-scope="scope">
              <span  >{{scope.row.userMoney}}</span>
        </template>
      </el-table-column>

          <el-table-column  min-width="100px" align="center" label="好评率">
        <template slot-scope="scope">
              <span  >{{scope.row.eva}}</span>
        </template>
      </el-table-column>
        <el-table-column  min-width="100px" align="center" label="注册时间">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" label="状态" width="100">
        <template slot-scope="scope">
          <el-tag   v-if="scope.row.username!=null" >已实名</el-tag>
          <el-tag  v-else type="danger">未实名</el-tag>
        </template>
      </el-table-column>
 ...
 <el-dialog title="用户信息" :visible.sync="dialogFormAuth">
   ...
   <el-form-item label="身份证" prop="describe">
    <Upload v-if="temp.audit==1"  :imageUrl="temp.images_card" v-model="temp.images_card" >
             ...
             <Upload v-else  :imageUrl="image_prefix+temp.images_card" v-model="temp.images_card" >
            </Upload>
      </el-form-item>
       <el-form-item label="暂住证" prop="describe">
    <Upload v-if="temp.audit==1" :imageUrl="temp.images_url" v-model="temp.images_url" >
            </Upload>
               <Upload v-else :imageUrl="image_prefix+temp.images_url" v-model="temp.images_url" >
            </Upload>
      </el-form-item>
       </el-form>
       <div slot="footer" class="dialog-footer">
...
<script>
import { getUserListbyData,statusUser,  updateArticle,setUserbyUpdate } from '@/api/user'
...
export default {
  ...
  data() {
    return {
       sortOptions: [{ label: '注册时间查询', key: '1' }, { label: '余额查询', key: '2' }, { label: '好评率', key: '3' }],
        fileList3: [{name:'',url:''}],
      diaimg:"",
      tableKey: 0,
      list: null,
  ...
  methods: {
    ...
    getList() {
      ...
      getUserListbyData(this.listQuery).then(response => {

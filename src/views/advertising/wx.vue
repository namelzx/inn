<template>
  ...
  <div class="filter-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="150px" align="center" label="图片">
        <template slot-scope="scope">
          <!-- <span class="link-type" @click="handleUpdate(scope.row)">{{scope.row.title}}</span>
          <el-tag>{{scope.row.type}}</el-tag> -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
        <el-form-item label="文章名" prop="skills_name">
      ...
      <el-form-item label="url" prop="icon">
          <el-input v-model="temp.url"></el-input>
        </el-form-item>

             <el-form-item label="公众号" prop="icon">
          <el-select v-model="temp.ar_type" placeholder="请选择">
              <el-option
                v-for="item in options"
              :key="item.id"
                            :label="item.articels_name"
             ...
             <el-form-item label="文章配图" prop="describe">
...
<script>
// import { getlist,statusUser,update,create,Userlevel,Delete,userlist } from '@/api/advertising'
import {wxfetchList,ConfigTask} from '@/api/articleconfig'
import {Articlecreate,getwxArticleBylist,Delete } from '@/api/article'
...
components:{Upload},
  ...
  methods: {
    getList() {
      ...
      getwxArticleBylist(this.listQuery).then(response => {
    ...
    getdata(){
         wxfetchList(this.listQuery).then(response=>{
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        ...
        if (valid) {
          Articlecreate(this.temp).then(response => {
        ...
        this.temp.images_url=this.image_prefix+  this.temp.images_url;
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        ...
        Articlecreate(tempData).then((res) => {

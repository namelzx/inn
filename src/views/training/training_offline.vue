<template>
  <div class="app-container">
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="150px" label="手机号码">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="110px" align="center" label="地区">
        <template slot-scope="scope">
        ...
        <template slot-scope="scope">
         ...
         <el-tag  v-if="scope.row.status =='0'" type="danger">未审核</el-tag>
          <el-tag type="success"  v-if="scope.row.status=='1'">成功预约</el-tag>
          <el-tag type="danger" v-if="scope.row.status=='2'">预约失败</el-tag>
        
        </template>
      </el-table-column>
      <el-table-column align="center" label="操作" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
        <el-button  size="mini" type="success" @click="handlemakeSatus(scope.row,scope.row.phone,1)">预约成功</el-button>
        <el-button  size="mini" type="danger" @click="handlemakeSatus(scope.row,scope.row.phone,0)">预约失败</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { logTraining,Delete,setTrainingSk} from '@/api/training'
...
export default {
  ...
  methods: {
      ...
      logTraining(this.listQuery).then(response => {
    ...
    // 预约状态
     handlemakeSatus(row,phone,status){
       this.TrainingSk.id=row.id
       this.TrainingSk.phone=phone
       this.TrainingSk.status=status
       setTrainingSk(this.TrainingSk).then(response=>{
         console.log(response);
          row.status =status
       })
    },
